...
import type { BotPerformanceReport, BotPerformanceRow } from "@/components/pages/analytics/activity-insights/bot-activity-insights-data";
import { cn } from "@/lib/utils";

const rowsPerPage = 5;
const emptyBots: BotPerformanceRow[] = [];
  ...
  return (
    <tr>
      <td colSpan={7} className="h-72 px-5 py-10 text-center font-inter text-sm text-muted-foreground">
        No bot performance data is available for the selected date range.
      </td>
    </tr>
  );
}

function getMailerTarget(bot: BotPerformanceRow, mailer: "gmail" | "domain" | "mask", daysTracked: number) {
  return bot.mailerTargets[mailer] * daysTracked;
}
...
function getAvailabilityStart(today: Date) {
  if (today.getDate() <= 7) {
...
export function BotPerformanceTableCard({
  report,
  dateRange,
  onDateRangeChange,
  onDownload,
  ...
  const bots = report?.bots ?? emptyBots;
  const daysTracked = report?.range.daysTracked ?? 0;
  const today = new Date();
  const availabilityStart = getAvailabilityStart(today);
  const availabilityEnd = today;
    ...
    <Card className="gap-0 rounded-xl border border-border bg-card py-0 shadow-sm shadow-black/5 ring-0">
      <CardHeader className="border-b px-5 py-4 pt-6">
        <div className="flex flex-col gap-4 xl:flex-row xl:items-center xl:justify-between">
          <div>
            <h2 className="font-google-sans text-xl font-semibold text-heading">Bot Employee Performance</h2>
            <p className="font-inter text-sm text-muted-foreground">
              Analyze automated bot sending performance for a selected date range.
            </p>
          </div>
          ...
          <div className="flex flex-col gap-2 sm:flex-row sm:flex-wrap sm:justify-end">
            ...
            <DropdownMenu>
              <DropdownMenuTrigger asChild>
                <Button disabled={bots.length === 0} className="h-10 rounded-full border-none bg-black px-4 font-google-sans shadow-sm shadow-black/10 transition-all duration-200 ease-in-out hover:bg-black/80 hover:shadow-md hover:shadow-black/20">
          ...
          <table className="w-full min-w-250 border-collapse">
            ...
            <tbody>
              {visiblePerformance.length === 0 ? (
                ...
                visiblePerformance.map((bot) => (
                  <tr key={bot.botId} className="border-b border-border transition-colors last:border-b-0 hover:bg-secondary/40">
                    ...
                    <td className="px-5 py-4"><span className="digits text-lg font-medium text-heading">{bot.monthTarget.toLocaleString()}</span></td>
                    <td className="px-5 py-4"><span className="digits text-lg font-medium text-heading">{bot.totalSent.toLocaleString()}</span></td>
                    <td className="px-5 py-4"><span className="digits text-lg font-semibold text-blue-600">{bot.completionRate}%</span></td>
                    <td className="px-5 py-4"><RatioValue sent={bot.mailerTotals.gmail} target={getMailerTarget(bot, "gmail", daysTracked)} /></td>
                    <td className="px-5 py-4"><RatioValue sent={bot.mailerTotals.domain} target={getMailerTarget(bot, "domain", daysTracked)} /></td>
                    <td className="px-5 py-4"><RatioValue sent={bot.mailerTotals.mask} target={getMailerTarget(bot, "mask", daysTracked)} /></td>
                  </tr>
                ))
              )}
            </tbody>
