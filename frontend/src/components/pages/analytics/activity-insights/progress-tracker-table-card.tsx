...
import type { ProgressTrackerRow } from "@/lib/activity-insights/activity-insights-api";
...
function TableLoader() {
  return (
    <tr>
      <td colSpan={5} className="h-72 px-5 py-10 text-center">
  ...
  return (
    <tr>
      <td colSpan={5} className="h-72 px-5 py-10 text-center font-inter text-sm text-muted-foreground">
        No employee progress is available for today.
      </td>
    </tr>
  );
}

export function ProgressTrackerTableCard({ rows, isLoading }: { rows: ProgressTrackerRow[]; isLoading?: boolean }) {
  ...
  return (
    <Card className="gap-0 rounded-xl border border-border bg-card py-0 shadow-sm shadow-black/5 ring-0">
      <CardHeader className="border-b px-5 py-4 pt-6">
        <div>
          <h2 className="font-google-sans text-xl font-semibold text-heading">Progress Tracker</h2>
          <p className="font-inter text-sm text-muted-foreground">
            Review each user&apos;s sent volume against their assigned daily limits.
          </p>
        </div>
      </CardHeader>
