...
const mailerType = MAILER_TYPE.GMAIL;
...
export function GmailMailerDashboard() {
  ...
  const sendMutation = useMutation({
    mutationFn: async (input: MailerSendPayload) => {
      ...
      if (result.status === "failed") {
        throw new Error("Gmail mail failed to send.");
      }
    ...
    onSuccess: (result) => {
      if (result.status === "partial") {
        toast.success("Gmail mail partially sent. Review delivery status.");
      } else {
        toast.success("Gmail mail sent successfully.");
      }
      setCooldownRemaining(cooldownSeconds);
      void queryClient.invalidateQueries({ queryKey });
    },
    onError: (error) => toast.error(error instanceof Error ? error.message : "Unable to send Gmail mail."),
  });
  ...
  return (
    <div className="flex flex-1 flex-col gap-6 p-4 lg:p-6">
      <GmailMailerStatusSection status={displayedStatus} isLoading={query.isLoading} cooldownTotalSeconds={cooldownSeconds} />
      <GmailMailComposer
